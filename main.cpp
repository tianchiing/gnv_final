  ...
  void initSystem(int argc, char * argv[])
  {
	...
	if (argc > 1) {
		...
		if (argc > 2) {
			...
			if (step > 0) {
				h = step;
				cout << "timestep is set to be " << h << endl;
			}else {
				cout << "invalid timestep, using default 0.04" << endl;
			}
		}else {
			cout << "using default 0.04" << endl;
		}
		if (type[0] == 'e'){
			timeStepper = new ForwardEuler();
  ...
  void drawSystem()
  {
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
		...
		case 't':
		{
			...
			if (toggle%3 == 0) {
				system = new SimpleSystem();
				cout << "Toggled to Simple System" << endl;
			}else if (toggle%3 == 1) {
				system = new PendulumSystem(4);
				cout << "Toggled to Pendulum System" << endl;
			}else {
				system = new ClothSystem();
				cout << "Toggled to Cloth System" << endl;
			}
			break;
		}
		case 's':
		{
			((ClothSystem *)system)->moving = !((ClothSystem *)system)->moving;
			cout << "moving / stop" << endl;
			break;
		}
		case 'w':
		{
			((ClothSystem *)system)->wind = !((ClothSystem *)system)->wind;
			cout << "wind / still" << endl;
			break;	
		}
        default:
